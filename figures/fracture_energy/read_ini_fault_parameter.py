...
if __name__ == "__main__":
    ...
    if "tractions" in parameters:
        try:
            ...
            out2 = easi.evaluate_model(
                centers, tags, ["rho", "mu", "lambda"], args.yaml_filename
            )
            out = {"rho":out2["rho"], "Vs": np.sqrt(out2["mu"]/out2["rho"]), "Vp": np.sqrt((out2["lambda"] + 2*out2["mu"])/out2["rho"])}
    else:
        out = easi.evaluate_model(centers, tags, parameters, args.yaml_filename)
        # add the last time step of ASl and Mud (for computing fracture energy)
        out |= {"ASl": sx.ReadData("ASl", sx.ndt-1), "Mud": sx.ReadData("Mud", sx.ndt-1)}
    sxw.write(
        args.output_file[0],
        sx.xyz,
        sx.connect,
