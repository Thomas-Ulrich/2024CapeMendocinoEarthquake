...
import numpy as np
import pygmt
from fault_plane_plotting import (
    plot_pygmt_fault_planes,
    plot_pygmt_fault_planes_background,
    plot_pygmt_fault_planes_trace,
)
...
kinematic_inv_gnss_syn = pd.read_csv("./gnss_data_preferred_DR.csv")
...
fig.coast(
    frame=["WSen", "xa0.5f0.1+lLongitude", "ya0.5f0.1+lLatitude"],
    projection=f"M20",
    region=region,
    shorelines="1p,black",
    land="lightgray",
...
if plot_kinematic:
    ...
    fig.text(
        x=-125.25, y=40.84, text="Kinematic inversion", font="10p", offset="3.5c/0.0c"
    )
...
fig.text(
    x=-125.25, y=40.87, text="Dynamic rupture model", font="10p", offset="3.8c/0.0c"
)
